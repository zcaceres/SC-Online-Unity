    ...
    public class CameraShaker : MonoBehaviour
    {
        ...
        void Awake()
        {
			...
			if (instanceList.Count == 0) {
				instanceList.Add (gameObject.name, this); // prevents Key value error on client login
			}
			cameraPosition = gameObject.transform.parent.transform.localPosition; //Gets local transform position for snapping by the shaker
        }
